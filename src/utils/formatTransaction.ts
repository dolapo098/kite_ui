...
export function formatTransactionAmountSummary(item: TransactionItemResponse): string {
  if (
  ...
  return formatBalanceCentsForDisplay(item.amount_in_cents, item.source_currency_code);
}
